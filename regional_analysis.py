...
selected_regions = st.sidebar.multiselect("분석 대상 지역", all_regions, default=["전국", "서울특별시"] if "전국" in all_regions else [all_regions[0]])

st.title(f"📊 {region_level} 단위 데이터 분석")

VARIABLES_MAP = {
    "1. 인구 및 사회경제": ["총인구수", "근로소득", "인당근로소득"],
    "2. 정신건강": ["자살률", "우울경험", "스트레스"],
    "3. 의료/예산/기타": ["치료_", "입원및외래_", "정신의료기관", "결산", "예산", "비만", "흡연"]
}
