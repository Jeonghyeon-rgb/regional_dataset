...
def load_combined_data():
    ...
    try:
        ...
        # 2. 경제 데이터 컬럼명 정리 (병합을 위해 '시도'로 통일)
        if '시도별' in sido_e.columns:
            sido_e = sido_e.rename(columns={'시도별': '시도'})
...
# --- 2. 변수 매핑 (요청하신 대로 경제 지표를 1번 카테고리에 통합) ---
VARIABLES_MAP = {
...
def get_unique_vars(keywords, df):
    ...
    # 세부 항목(계/남자/여자 등)이 있는 경우를 위해 get_base_name 활용
...
st.info("💡 '1. 인구 및 사회경제적 배경'에서 새로 추가된 경제 지표(GRDP, 성장률 등)를 확인할 수 있습니다.")
...
def process_data_v2(df, regions, var_name, loc_column):
    ...
    def extract_year(text):
        ...
        if match:
            ...
            return int(full_year) # 정렬을 위해 정수로 변환
        return None
...
if selected_all_vars and selected_regions:
    ...
    if "평균 추이" in view_mode:
        for i, var in enumerate(selected_all_vars):
            ...
            # 단위가 다른 지표(예: GRDP vs 자살률)를 위해 이중 축 사용
        ...
        layout_update = {
            "xaxis": dict(title="연도", dtick=1, gridcolor='whitesmoke'),
        ...
        fig.update_layout(title=f"지역별 '{target_var}' 추이 비교", xaxis=dict(dtick=1), template="plotly_white")

    st.plotly_chart(fig, use_container_width=True)
else:
    st.info("👈 왼쪽에서 지역을 선택하고, 상단에서 비교하고 싶은 지표(예: GRDP와 자살률)를 클릭하세요.")
